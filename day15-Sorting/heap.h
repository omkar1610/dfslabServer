...
//Generic swapping ith and jth using 0 as variable
...
void showFullArrInt(HEAP *h, int used_tot) //Print the full array irespective of num used
{
	printf("HEAP : ");
	for(int i = 1; i<=used_tot;i++)
	{
		int *ele = (int*)(h->array + i * h->element_size);
		printf("%d ", *ele);
	}
...
void extractMax(HEAP *h) // Deletes max and keep at last for future printing
{
	// h->array[0] used as swapping variable in one-based indexing
	void *ip =  h->array + h->element_size;
	void *jp =  h->array + h->num_used * h->element_size;
	void *tp =  h->array;
	// Memory to memory copy of the elements for swapping
	memcpy(tp, ip, h->element_size);
	memcpy(ip, jp, h->element_size);
	memcpy(jp, tp, h->element_size);
